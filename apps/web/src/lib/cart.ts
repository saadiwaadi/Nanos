...
/**
 * Client-side cart store (localStorage-backed) — the UI-facing half of the
 * cart until the CartModule API lands. Every page reads the same state via
 * useCart(); mutations persist instantly and broadcast so the header badge,
 * cart page and checkout summary stay in sync without navigation.
 *
 * When the CartModule API exists, swap the mutators for fetch calls and keep
 * the same hook signature — consumers won't change.
 */
...
const STORAGE_KEY = "nanos_cart_v1";
...
export function useCart() {
  ...
  return {
    ...
    addItem(item: Omit<CartItem, "qty">, qty = 1) {
      ...
      const existing = cur.items.find((i) => key(i) === key(item as CartItem));
      if (existing) existing.qty += qty;
      else cur.items.push({ ...item, qty });
      save(cur);
    },
    ...
    updateQty(productId: string, color: string, size: string, delta: number) {
      ...
      if (!it) return;
      it.qty += delta;
      if (it.qty <= 0) cur.items = cur.items.filter((x) => x !== it);
      save(cur);
    },
    ...
    removeItem(productId: string, color: string, size: string) {
      ...
      if (cur.items.length === 0) cur.promo = null;
      save(cur);
    },
