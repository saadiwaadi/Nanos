...
function SizeGuidePanel({ onClose }: { onClose: () => void }) {
...
export function ProductDetailPanel() {
  ...
  useEffect(() => {
    ...
    const closeBtn = panel.querySelector<HTMLElement>(".pdp-close");
  ...
  return (
    <>
      {/* overlay backdrop */}
      <div
        className="pdp-panel-backdrop open"
        onClick={(e) => {
      ...
      <div
        ref={panelRef}
        className="pdp-detail-panel open"
        role="dialog"
        aria-modal="true"
        aria-label={product.name}
      >
        {/* handle/bar at top */}
        <div className="pdp-panel-handle" />
        ...
        <div className="pdp-panel-inner">
          ...
          <div className="pdp-details-col">
            ...
            <p className="pdp-sub">
              {categoryLabel} · {product.colors.length} colors ·{" "}
              {product.sizes.length} sizes
            </p>

            <div className="pdp-price-row">
              <span className="pdp-price">{fmtPrice(product.price)}</span>
            ...
            {product.tag && (
              <span className={`pdp-badge pdp-badge-${product.tag.toLowerCase()}`}>
                {product.tag}
              </span>
            )}
            ...
            <div className="option-group">
              ...
              >
                <ChevronDown size={14} style={{ marginRight: 4, verticalAlign: "middle" }} />
                Size Guide
              </button>
            </div>
            ...
            <div className="pdp-actions">
              <button
                type="button"
                className={`pdp-add-to-cart${added ? " added" : ""}`}
                onClick={handleAdd}
      ...
      {/* nested size guide modal */}
      {sizeGuideOpen && (
        <SizeGuidePanel onClose={ctx.closeSizeGuide} />
      )}
    </>
  );
}
